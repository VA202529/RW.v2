...
function AdminClients() {
  ...
  return <PagePanel title="Klanten" subtitle="Zoek klanten, tegoeden, notities en blokkades." actions={<input placeholder="Zoeken" value={q} onChange={e => { setQ(e.target.value); load(e.target.value); }} />}><div className="table">{customers.length === 0 ? <EmptyPanel>Geen klanten gevonden.</EmptyPanel> : customers.map(c => <div className="tableRow clientRow lovableDataRow" key={c.id}><span>{c.full_name || "-"}</span><span>{c.email}</span><span>{c.phone_e164 || "-"}</span><span>{c.visit_count} bezoeken</span><span>{c.last_visit_at ? formatLocal(c.last_visit_at) : "-"}</span><span>{cents(c.credit_cents)}</span><label><input type="checkbox" checked={c.is_blocked} onChange={e => update(c, { is_blocked: e.target.checked })} /> Geblokkeerd</label><input defaultValue={c.notes || ""} onBlur={e => update(c, { notes: e.target.value })} /></div>)}</div></PagePanel>;
}
...
function AdminStats() {
  ...
  return <PagePanel title="Statistieken" subtitle="Live omzet, boekingen en klanttrends." actions={<><input type="date" value={from} onChange={e => setFrom(e.target.value)} /><input type="date" value={to} onChange={e => setTo(e.target.value)} /><button className="primary" onClick={load}>Vernieuwen</button></>}>{stats ? <div className="metrics adminMetrics lovableMetrics"><MetricCard label="Boekingen" value={stats.bookings.total} /><MetricCard label="No-show" value={`${stats.no_show_pct}%`} /><MetricCard label="Aanbetalingen" value={cents(stats.deposit_revenue_cents)} /><MetricCard label="Platform fee" value={cents(stats.platform_fee_cents)} /><MetricCard label="Terugkeer" value={`${stats.return_rate_pct}%`} /><MetricCard label="Nieuwe klanten" value={stats.new_customers} /></div> : <EmptyPanel>Statistieken laden...</EmptyPanel>}</PagePanel>;
}
